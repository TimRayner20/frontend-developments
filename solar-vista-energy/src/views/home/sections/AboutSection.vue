<script setup lang="ts"></script>

<template>
  <div
    id="about-us"
    class="about-section relative my-5 bg-[#34495e] lg:mx-auto text-left lg:text-left py-8 lg:py-16 mb-0"
  >
    <div
      class="container flex flex-col lg:flex-row text-white py-5 px-5 m-auto w-full"
    >
      <div
        class="text-center lg:text-left text-wrapper flex flex-col w-full lg:w-1/2 pr-5 align-middle m-auto"
      >
        <h3 class="text-4xl text-center lg:text-left font-bold">
          About Solar Vista
        </h3>
        <p class="my-2 mb-8">
          Hi 👋, we're Solar Vista! We're on a mission to help like minded
          businesses reach net zero before 2050 by offering bespoke solar
          solutions. We're a team of passionate individuals who are committed to
          making a difference. Want to read more about us? Click the button.
        </p>
        <a
          href="/about"
          class="btn block bg-[--primary] text-white rounded-lg p-3 w-2/3 mb-6 m-auto lg:m-0 lg:w-4/12 text-center"
        >
          Read more
        </a>
      </div>
      <div class="img-wrapper lg:m-0 lg:ml-auto">
        <img
          src="../../../assets/images/night-neighborhood.png"
          alt="Solar Solutions"
          class="rounded-xl w-full lg:ml-auto"
        />
      </div>
    </div>
  </div>
</template>
